/**
 * zip-code-analyzer.js (Advanced + Export)
 *
 * Features:
...
function exportMarkdown(reportName, allReports) {
...
function exportJson(reportName, allReports) {
    const jsonData = {
        meta: {
    ...
    try {
        ...
        for (const entry of entries) {
            ...
            if (!isAllowedExtension(entry.name)) {
                ...
                // Add to report even if skipped for statistics
                allReports.push({
                    filename: entry.name,
                    status: 'skipped',
        ...
        if (CONFIG.enableReporting) {
            ...
            if (CONFIG.outputFormats.includes('md')) {
                exportMarkdown(path.basename(zipPath), allReports);
            }
            if (CONFIG.outputFormats.includes('json')) {
                exportJson(path.basename(zipPath), allReports);
            }
        }
